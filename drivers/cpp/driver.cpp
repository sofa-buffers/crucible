...
// The receiver's field-span budget (CORELIB_PLAN §6.2.1). Since corelib-cpp#128 the
// pure-C++ corelib takes it as a CONSTRUCTOR ARGUMENT with no default — §6.2.1 leaves
// the library no number to invent — so every stream this driver builds by hand has to
// state it, and it must state the SAME number the generated `try_decode` states or the
// chunked path would decode under a different policy than the one-shot path and the
// chunk-invariance gate would report the difference as a divergence.
//
// The generated header defines SOFAB_MAX_DYN_BUFFERED_FIELD only when the build
// configured receiver caps (limit mode); without them the generated code passes
// SIZE_MAX literally, so that is what we mirror. The c-cpp wrapper's IStreamInline
// takes no such argument at all, which is why the macro below gates its use.
#ifdef CRUCIBLE_ISTREAM_TAKES_LIMITS
#ifdef SOFAB_MAX_DYN_BUFFERED_FIELD
static const sofab::Limits kReceiverLimits{SOFAB_MAX_DYN_BUFFERED_FIELD};
#else
static const sofab::Limits kReceiverLimits{SIZE_MAX};
#endif
#endif
...
static void decode_and_report(const std::uint8_t *data, std::size_t len, FILE *out)
{
    ...
    if (len > 0)
    {
        ...
        sofab::IStreamObject<message::Probe> in{kReceiverLimits};
