...
def _emit_leaf(em, kind, expr):
    if kind == "u":
        ...
        em.stmt(f"_bytes(b, 't', _strBytes({expr}));")
    elif kind == "blob":
        em.stmt(f"_bytes(b, 'b', _blobBytes({expr}));")
    else:
...
// A decoded fp32 array is a Float32List (or, since generator#570, an InlineFloat32Array
// backed by one), whose byte buffer still holds the untouched little-endian wire bits;
// fall back to the repack only for a plain List<double>.
void _f32Elem(StringBuffer b, Object a, int i) {
  final l = a is sofab.InlineFloat32Array ? a.storage : a as List<double>;
  if (l is Float32List) {
...
// Since generator#570 a string/blob is decoded into a corelib-dart inline destination
// (InlineString / InlineBytes) — only its first `length` bytes are in use. A string is
// read as the UTF-8 bytes the decoder stored, never re-encoded from a Dart String.
List<int> _strBytes(Object v) => v is String ? utf8.encode(v) : _blobBytes(v);

List<int> _blobBytes(Object v) => v is sofab.InlineBytes
    ? Uint8List.sublistView(v.storage, 0, v.length)
    : v as List<int>;

void _bool(StringBuffer b, Object v) {
  if (v is bool) {
    b.write(v ? 'u1' : 'u0');
    return;
  }
