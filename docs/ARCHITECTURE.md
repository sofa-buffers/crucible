...
| `scripts/materialize.sh` | built | Runs the materialized differential over the **full 12-driver roster** with `SOFAB_MATERIALIZE=1`, over `corpus/structured` — **75×12 → 0 divergences**. C is the schema-agnostic anchor (object-descriptor walk, `drivers/c/driver.c`); the other 11 hand-walk the decoded value with a schema-type table (a generated table would make them schema-agnostic too — the one open refinement). |
