...
> **Status: Phases 1–3 largely done** — the differential loop runs across all seventeen
> drivers / twelve corelibs (C pacemaker, Go, Rust-std, Rust-no-std, **four C++
> configurations** — two corelibs × both `allow_dynamic` settings — Python-Cython,
> Python-pure, Java, TypeScript, C#, Zig, Dart, **and two Kotlin Multiplatform
> targets** — one corelib compiled for the JVM and for Kotlin/Native) over the
> full-scale `probe` schema.
> The roster itself is `drivers/roster`; no driver is currently quarantined, so all
> seventeen run in the blocking gates. Phase 3 is built (structure-aware mutator, round-trip + cross-encode
...
| `schema/probe.sofab.yaml` | built | **Full-scale** message (Phase 3): 8 scalar widths, fp32/fp64, string, blob, 8 numeric arrays, nested fp arrays, string array, blob array, and (WP-05, 2026-07-27, poc family) `struct_array` — the array-of-struct (`{k: u32, v: string}`, id 202) whose elements are sequences, the position the §2/§5.1 sequence-element rules are tested at, and (2026-09-18) four **`boolean`** positions — a root scalar (id 203), a root inline `array of boolean` (id 204, count 5), a struct child (`nested`, child id 9) and a wrapper-element child (`struct_array` element field `f`, id 2). Still keyed `probe` (stable type name). |
| `schema/probe-union.sofab.yaml` | built | `probe` message carrying a **union** (`choice`: `as_u16`/`as_i32`/`as_text`/`as_blob`) between a scalar `tag` and `trailer` — the one wire feature the full-scale `probe` lacks. Drives `scripts/run-union.sh` (differential) **and, since WP-01 (2026-07-22), the union pass of the structural sweeps** (`sweep_run.py --union`), which rebuilds the roster against it and back. |
...
| `scripts/materialize.sh` | built | Runs the materialized differential over the **full blocking roster** (`drivers/roster`) with `SOFAB_MATERIALIZE=1`, over `corpus/structured` — **0 divergences** (agreement) **+ a C-anchor conformance check** vs the reference (a family-wide-wrong dump is agreement-green, conformance-red). **Before the differential**, an anchor-vocabulary check (`materialize.py --anchor-vocab`) fails on any `?` in the C anchor's dumps — `?` is outside the materialized grammar and comes only from `md_value()`'s `default:` arm, i.e. a field-type tag the anchor has not learned — so an anchor that is behind reports as `ANCHOR BEHIND`, not as a family-wide divergence. **A standing CI gate** (`replay.yml`); exports `SOFAB_MATERIALIZE_SCHEMA` for the descriptor-driven drivers. **Every walker is schema-agnostic:** C (sofabgen object descriptor); **go/ts/java/cs/python** consume the generated `materialized-schema.json` at runtime (reflection); **rust/cpp/zig/dart/kotlin** — no runtime reflection — instead **generate their walker source at build time** from the descriptor (`drivers/<lang>/materialize_gen.py`, run by `build.sh`, unrolling the descriptor into straight-line access code). Kotlin is in the generated camp even though its JVM leg *could* reflect: `kotlin-reflect` is JVM-only, so a reflecting walker would need a second implementation for the native leg — and one source across the targets is the whole point of the driver. A schema change reflows to the whole roster with zero hand-editing. |
| `oracle/comparator.py` | built | N-way canonical diff, policy-aware, no external deps; parses `A`/`I`/`R`. **Crash- and hang-isolating:** a per-driver wall-clock budget (`--timeout`, default `max(30s, 0.25s × corpus)`; `TIMEOUT=` env via the scripts) via stdout-to-tempfile, so an adversarial input that hangs a driver is localized + reported `[TIMEOUT]` (a DoS finding), not a wedged run. `read_corpus` skips `*.md` + dotfiles so a corpus dir can carry a README (inputs can't be selected by extension — libFuzzer names files by content hash); this also stops the `.gitkeep` in the gitignored corpora being fed as an empty message. **Since 2026-08-17 it also judges each line's reject class on its own**, independently of what the other drivers reported: CORELIB_PLAN §6.3 fixes the taxonomy at five codes and abolishes the "invalid usage" category outright, and a class that must never appear cannot be policed by the `reject_class` axis — that one fires on disagreement, so a forbidden class named by *every* implementation would be unanimous and therefore green. `usage` is forbidden and fails the run; `argument` / `buffer_full` / `other` are legal under §6.3's language-specific allowance but reported as warnings, because on the decode path each means a generated layer erred where the family cleanly rejects. Grouped per (driver, class) with a count and one example, since over a fuzzed corpus one class can be named thousands of times. `oracle/canonical.md` owns the split. |
| `oracle/policy.yaml` | built | **What counts as a bug.** Two parts, both read by `comparator.py`. (1) The five **axes**. Four are hard — a difference in the verdict, in an accepted value, in the reject **class** or in the limit class fails the run. `reject_class` and `limit_class` went hard on 2026-08-16 after being measured: across `corpus/interesting` (17870 inputs), regression, structured, conformance, seeds and limit mode they fire **zero** times, so the leniency protected nothing and would have hidden the first real disagreement. Only `incomplete_value` stays soft, and not as legacy: it fires 5471 times, every one of them a JVM-family driver — `java` and, since the Kotlin target joined, both Kotlin legs — handing back what it had already read when the stream ended mid-field while the other fourteen hand back nothing — unanimous verdict, differing payload. (2) The **allow list**: named inputs that may differ on one named axis because the difference follows from a language's design — today the C NUL-terminated-string projection (F-0018), plus one entry deliberately dormant. **Enforced since 2026-08-16**, matched by the input's **bytes**, never its path, so an allowance survives promotion into a corpus; a path that no longer resolves is reported. |
...
| `scripts/fuzz.sh` | built | The C pacemaker: build the libFuzzer target (clang) + run + grow corpus/interesting. `FUZZ_STREAM=1` (crucible#178) builds and runs a second, independent `LLVMFuzzerTestOneInput` in the same `driver.c` — `CRUCIBLE_FUZZ_STREAM` — that steers on the STREAMING (`feed`/`finish`) decode path instead of the block-path decode above; nothing else fuzzes that path, every other front-end calls the one-shot decode, so it was only ever *replayed*. Its input is a 3-byte `[mode][param_lo][param_hi]` header in front of the message, mirroring the replay driver's own two axes (`mode&1==0` → `SOFAB_SPLIT=param`, `==1` → `SOFAB_CHUNK=param`), so a violation it finds reproduces directly against the replay driver with that variable. It is not a crash-only target: every streaming defect found so far (F-0058, F-0060, F-0061, crucible#130) was a value or verdict mismatch, so each input is decoded twice — one-shot and chunked per its header, through the SAME `decode_chunked` helper the replay driver's `SOFAB_SPLIT`/`SOFAB_CHUNK` path calls — and the run aborts on a verdict-class or re-encoded-bytes mismatch (an intra-driver check, the fuzz-time form of `run-chunked.sh`'s own invariant). The chunked leg always scrubs each fed chunk afterward, so a decoder that borrowed rather than copied is caught the same way `SOFAB_CHUNK_SCRUB` catches it in the replay driver. Its own accumulating corpus is `corpus/stream/` (header-prefixed inputs, not readable by any other driver); harvesting strips the 3-byte header and merges the bare message into `corpus/interesting/` by content hash, so the shared corpus never carries a header. The header-aware custom mutator (below) keeps the grammar-aware ops from corrupting the cut position by mistaking it for wire bytes. |
...
| `scripts/fuzz-go.sh` + `drivers/go/{fuzz_test.go,gocorpus.py}` | built | **Second steering engine** — Go's native coverage-guided fuzzer (`go test -fuzz`, no external framework) over `corelib-go`'s decoder. The pacemaker steers by *C* coverage and so only explores C-complex paths; this steers by Go's and feeds discoveries into the same `corpus/interesting`. Go stores its corpus (seed *and* the coverage corpus under `$GOCACHE`) in a text format rather than raw bytes, in both directions; `gocorpus.py` is the **only** place that format is understood — it writes every byte as `\xNN` (always a valid Go literal) and, when reading, handles the richer set Go itself emits, including `\u`/`\U`, which name a code point and therefore contribute its multi-byte **UTF-8 encoding**. Seeds are named `seed_<sha1>` so anything else left in `testdata/` afterwards is by construction a Go-written failing input → `corpus/crashes/`. Runs in `nightly.yml` after the C pass at a quarter of its budget. `FUZZ_TARGET=FuzzProbeStream` (default `FuzzProbe`; crucible#178) switches both the build and the harvest to a second target, `FuzzProbeStream`, that steers on `sofab.Decoder.Feed` — the STREAMING path — instead: `corelib-go` gained a resumable push decoder since the `chunked_decode=none` declaration below was written (§0 of the crucible#178 plan), so a replay-driver streaming axis is now possible there too, tracked separately; this target uses the corelib's `Feed` directly. Its cut position is two further Go fuzz arguments (`mode uint8`, `param uint16`), not a header in the bytes — Go mutates typed arguments independently — mirroring the same `SOFAB_SPLIT`/`SOFAB_CHUNK` axes as the C target above. Like the C target, it is not crash-only: it decodes one-shot and chunked and `t.Fatalf`s on a verdict-class or re-encoded-bytes mismatch, scrubbing each fed chunk after `Feed` returns. `-fuzz` is a **regex**, so an unanchored `FuzzProbe` also matches `FuzzProbeStream`; the script anchors it (`^…$`) for both targets. `gocorpus.py`'s strict single-argument `decode` stays exactly as before (used by `FuzzProbe`); `FuzzProbeStream`'s multi-argument entries go through the new `decode --first-bytes`, which returns only the `[]byte` payload (so the shared corpus never carries `mode`/`param`) and, given an `<args-file>`, the trailing arguments' decoded integer values — accepting both the numeric form this module writes (`uint8(1)`) and the quoted-rune form Go itself writes when a value is printable (`byte('\x7f')`, `byte('ý')`). |
| `oracle/cluster.py` | built | Groups divergences by camp-partition into root causes (`CLUSTER=1 ./scripts/run.sh`); 256 divergences → 47 clusters. With `--baseline` it diffs every camp against `results/known-clusters.txt` and exits non-zero on an unexplained one. **Rows match on the drivers they name** (since 2026-08-16): a signature written before a driver existed still matches, so adding one no longer invalidates the file — on 2026-08-05 it did, and read as "9 NEW CAMPS, 0/9 accounted for" with zero new root causes. Two things still turn a camp NEW: a named driver that changed side (real movement), and an unnamed driver sitting **alone** in a camp — that one is the divergence worth seeing when a driver is added, and the first version of the matching swallowed it. Where an unnamed driver joined an existing camp is printed per camp. The file's `# roster:` line is context, not a gate. |
...
| `corpus/stream/` | built | The C streaming engine's own accumulating corpus (crucible#178, `FUZZ_STREAM=1 ./scripts/fuzz.sh`): header-prefixed inputs (`[mode][param_lo][param_hi][message…]`), not readable by any other driver or gate. Harvesting strips the header and merges the bare message into `corpus/interesting/`, so this directory is a working corpus for the streaming steering engine, not a second shared corpus. |
...
| `engine/structured/audit_boolean.py` | built | **Hand-run coverage audit for §4.4** (2026-09-18), the sibling of `audit_canonical.py`: re-derives from the schema and the axes whether every place a `boolean` can sit has one, whether `sweep_positions` agrees with the schema, which axes reach a boolean position (and, for `sweep_truncation`, how many cut points land inside a boolean field — that axis names vectors by offset, so a name grep reports 0 and is wrong), whether all twelve materialized-form walkers handle the kind, and whether the value set still covers each distinct way of getting the clause wrong. Exits non-zero on a gap. |
| `engine/structured/sweep_tolerance.py` | built | The **tolerance axis** (§7.2 class 5b), blocking since 2026-08-03. **96 vectors**, in two families. *(a)* The **§4.4 boolean** family (2026-09-18, 47 vectors over the four boolean positions): the canonical `true` as the control, then `2`, `0xff`, `256`, `2^32` and `2^64-1` — every one of them `true` per §4.4, none of them `INVALID`, all `same:` the control, because a boolean is the one integer-backed leaf type with **no width bound** (MESSAGE_SPEC §1); plus `true` spelled non-minimally, `false` written explicitly (`same:` the field-absent twin, whose scopes are identical so §2 omission and §5.1 last-element framing need no modelling here), and an over-64-bit varint as the strict-side contrast. **Currently RED** — it is what found F-0064 / G-0042. *(b)* The **§4.9 sequence-end** family, 49 vectors over all 7 sequence positions: a sequence-end header carrying id 3 and id `ID_MAX`, and id 0 spelled non-minimally — each `same:` its canonical twin, so both halves of §4.9 are asserted (accept, *and* normalize to `0x07`) — against over-`ID_MAX` and over-64-bit-varint rejects as the strict-side contrast. Every vector closes a **declared** sequence holding a real field, because an empty frame is normalized away by §2 and would make the discarded id unobservable — the blind spot F-0054's own isolate had. The non-minimal *varint* half of class 5b stays in `sweep_varint` and is deliberately not duplicated here. `emit_union` carries the same seven vectors onto the **union** sequence in `schema/probe-union.sofab.yaml` — a union is an ordinary sequence on the wire, so §4.9 binds its closing marker too, but it lives in a schema the probe pass cannot reach. |
| `oracle/chunk_invariance.py` + `scripts/run-chunked.sh` | **oracle built, drivers pending** | The **chunk-invariance** gate (CORELIB_PLAN §7.2 item 4): for every input and every split point, feeding `[0,k)` then `[k,end)` into one decoder must produce the canonical line the whole message produces. Alone among the oracles here it is **not differential** — it compares a driver against itself, so it needs no second implementation, drivers opt in one at a time, and it is the only gate that can catch a defect the whole family shares. Also asserts **resumability**: an `I` must still reach the right verdict and value after the remaining bytes arrive. Drivers opt in via the variables and are named explicitly in `SOFAB_SPLIT_DRIVERS`, because a driver that ignores them emits identical output and would pass vacuously. **All three cuts are implemented here:** the `SOFAB_SPLIT=k` sweep over every interior boundary (which also says *which* boundary broke), `SOFAB_CHUNK=n` at sizes 1/2/3/5/8/16 (`n=1` splits every varint, length word and payload, so it cannot straddle the boundary that breaks), and `SOFAB_CHUNK_SCRUB=1` at `n=1` — a *lifetime* check rather than a boundary one, catching a decoder that borrows from a fed chunk instead of copying out of it. `--modes` selects a subset. **Which drivers implement it is declared in `scripts/run-chunked.sh`** — that list is its own owner, so it is not restated here. Each participating driver announces its configuration on stderr when a variable is set, which is what makes "it really re-feeds" checkable rather than asserted: stdout is identical either way. **Run twice in `replay.yml` since 2026-08-18**: over `corpus/seeds` with every mode, and over `corpus/regression` with `--modes chunk`. The second is where the chunk-boundary findings live — F-0058, F-0060 and F-0061 were promoted into that corpus but every gate replaying it feeds each record *whole*, so it held their one-shot verdict and not the behaviour they were about. Only the fixed-size cut, because the split sweep is O(maxlen) per driver and does not fit over 239 inputs; six chunk sizes still cross every internal boundary at six offsets. 239 x 6 x 14 drivers, ~10s on already-built drivers. |
| `oracle/encode_invariance.py` + `scripts/run-encode.sh` | built | The **encode-invariance** gate (crucible#132) — the encode-side twin of the above, and likewise **not differential**. The family is byte-canonical, so the three encode surfaces of one implementation (`SOFAB_ENCODE=new` → allocating `encode()`, `to` → caller-buffer `encodeTo()`, `stream` → `serialize(os)`) must emit identical bytes for the same decoded value, and `SOFAB_FLUSH=n` (an `n`-byte `OStream` buffer) must not change them — the encode-side mirror of `SOFAB_CHUNK=1`, walking the encoder across a buffer boundary at every offset. Which surfaces a backend has comes from `meta`'s `encode_surfaces`; the baseline every surface must reproduce is the driver's own **default** path, so a driver that reads the variable but wires it to the wrong call is caught too. It additionally asserts the contract's **hard-fail**: asking for a surface the backend lacks must exit non-zero, never fall back silently. Defaults to `corpus/structured` rather than `corpus/seeds` — only an accepted input re-encodes, so on `I`/`R` every surface trivially agrees. Opt-in via `SOFAB_ENCODE_DRIVERS`; **which drivers implement it is declared in `scripts/run-encode.sh`**, its own owner. **Which flush sizes are swept is the port's own declaration** (§5.1, rewritten by documentation#46/#48 on 2026-08-11): `meta`'s `min_output_buffer` carries the smallest streaming buffer the corelib accepts — `1` for a port that splits atomic units across a flush, otherwise the largest run it reserves as one piece, capped at `20`. The sweep is `{declaration} ∪ {1,2,3,5,8,16 above it}`, so it always contains the declaration itself and can never be empty; a refusal (exit **3**, distinct from 2, "no such surface") at or above the declaration is a conformance failure, and a size one byte *below* it is asserted to be refused, which is what stops a high declaration from being an escape hatch. Every port but `go` declares 1 and therefore sweeps the full 1/2/3/5/8/16 as before; `corelib-go` declares `2 × maxVarintLen` = 20. **It carries no pass-through axis.** Between 2026-08-17 and 2026-09-14 it did — `SOFAB_PASSTHROUGH=1`, exercising §5.1's optional permission to hand a `string`/`blob` run to the sink directly. CORELIB_PLAN **§5.1.6** withdrew that permission and forbade the behaviour outright, so the assertion inverted (prove it never happens, not prove it was exercised) and the per-port declaration lost its meaning: the answer is now the same for every port and fixed by the spec, not chosen by the implementation. The check moved into the drivers, where the sink actually lives — a driver tests every slice it is handed against the buffer it installed, reports `foreign sink handovers=<n>` at clean EOF and exits non-zero above zero — and this gate catches it through the exit code of the stream-surface runs it already makes. **Only `go` implements the assertion today** (`docs/TODO.md`). |
...
| `scripts/check-family-copies.py` | built | **The family-copies gate** (crucible#192): every place Crucible reimplements logic the family owns, listed with a verdict. Four checks. *(1)* Tracked sources under `drivers/ engine/ oracle/ scripts/` are scanned for four signatures — a varint/zigzag coder, a `case SOFAB_OBJECT_FIELDTYPE_*` dispatch, a materialized-value walk, a *defined* wrapper-array placement helper — and every hit must match an `INVENTORY` entry naming that file and signature with its reason, while every entry must still match (no stale claims); `findings/` reproducers are out of scope. Today's inventory: the C anchor, ten materialized walkers plus the reference and `schema.py`, and the two independent byte builders `gen.py` / `sweep_varint.py`. *(2)* Every `SOFAB_OBJECT_FIELDTYPE_*` the vendored corelib-c-cpp defines has a `case` in `md_value()`. *(3)* Every kind in `oracle/materialized-schema.json` is named by every walker. *(4)* Every roster driver's generated probe code calls its corelib's collector/placement layer at least once per wrapper field (generator#587), and every roster entry has such a declaration. `--static` runs (1) and (3) in the `catalog` job; the full run follows the seed `run.sh`, before any gate rebuilds against another schema. It is signature-based, so a copy spelled unlike any signature passes — it narrows the drift, it does not prove its absence. |
...
| `.devcontainer/` | built | Fuzzing toolchains (clang/libFuzzer, cargo-fuzz, Jazzer, Atheris, SharpFuzz, Jazzer.js). **The JVM toolchain is Temurin 21**, not the distro's `default-jdk`: Ubuntu 26.04 ships JDK 25 and Gradle 8.14 — what corelib-kotlin-mp's wrapper pins — will not run on it. `JAVA_HOME`/`PATH` point at `/opt/java-current`, since the Temurin package does not win `update-alternatives` against the distro JDK; `/usr/bin/java` is still 25 and is no longer what a build gets. Both JVM drivers therefore compile on 21 (corelib-java targets release 17, so this is invisible to it). Kotlin adds Gradle 8.14.5 (pinned to the wrapper's version) and `kotlinc` 2.4.10 (pinned to the `kotlin("multiplatform")` version in corelib-kotlin-mp), plus the **Kotlin/Native** distribution under `KONAN_DATA_DIR=/opt/konan` — the standalone `kotlinc` has no native front-end (its `bin/` is jvm/js/wasm only), so the `native` driver leg needs it. It is two downloads, not one: the prebuilt compiler, and the LLVM + sysroot dependencies it fetches **lazily on first compile**, which the image warms by building a hello-world. Both land in the layout Gradle uses, so the corelib's own Gradle build reuses this copy rather than pulling ~2 GB into the mounted workspace mid-gate. |
...
| `drivers/kotlin/` (**jvm + native**) | built | Kotlin Multiplatform driver against corelib-kotlin-mp (generator#340, the 11th sofabgen backend). **One `driver.kt` for two KMP targets**, the drivers/rust/ shape one level up: `build.sh <jvm|native>` builds the corelib for that target through its own Gradle build (`jvmJar` / `linuxX64MainKlibrary` — never by hand-compiling its sources, which would drop the JVM target and compiler flags `build.gradle.kts` pins) and compiles the driver against it directly with `kotlinc` / `kotlinc-native`, the reason `drivers/java/build.sh` calls `javac`. Registered as two rows (`kotlin-jvm`, `kotlin-native`) because they are two implementations to compare: one `commonMain` codec, but the little-endian word access is an `expect`/`actual` — byte-array `VarHandle`s on the JVM, indexed shifts through LLVM natively — so the wire behaviour being identical is a claim the differential can test rather than assume. The **only** target-specific driver code is the IO shim (`io_jvm.kt` / `io_native.kt`: the environment, binary stdin, stdout), because common Kotlin has no API for any of the three; everything else — the verdict mapping, the chunking, the three encode surfaces, the materialize walk — is shared. **Status-returning single-pass decode:** `Probe.tryDecode(ByteArray, Probe) → DecodeStatus` (`INCOMPLETE`→`I`, `COMPLETE`→`A`), with a thrown `SofabException`→`R`, its `SofabError` giving the class, and `LIMIT_EXCEEDED`→`L`. Heap profile (`MutableList`, grown arrays) → in the limit-mode roster. Coverage front-end is Jazzer on the JVM leg (`FuzzProbe.kt`, not built by `build.sh`); the native leg has none — Kotlin/Native exposes no libFuzzer entry point, the zig/dart position (PLAN §14). |
| `engine/mutator/` (structure-aware) | built | `sofab_mutator.{h,c}` — grammar-aware libFuzzer custom mutator (varint truncate/extend/flip/maxout, header type/id, fixlen length, array count, sequence open/close, invalid-UTF-8, fp NaN/inf, field dup). Wired via `LLVMFuzzerCustomMutator` in `drivers/c/driver.c` (~37% byte-mutator mix-in) + `scripts/fuzz.sh`. Pure/testable; `test_mutator.c` soak = 336k mutations, 0 OOB under ASan, deterministic. See DESIGN.md "As built". Under `CRUCIBLE_FUZZ_STREAM` (crucible#178) `driver.c` swaps in a header-aware variant that mutates EITHER the 3-byte mode/param header OR the message, never both in one call — otherwise these grammar-aware ops would read the header as wire bytes and corrupt the cut position along with the message; falls back to the plain byte-level mutator on the whole buffer whenever there is no room for a header. `sofab_mutator.{h,c}` itself is unchanged; the header-awareness lives in `driver.c`'s wrapper. |
...
by consuming the generated `materialized-schema.json` at runtime, rust/cpp/zig/dart/kotlin by
generating their walker source from the descriptor at build time: **111×17 → 0
...
- **rust (rs + rs-no-std)** — one shared `drivers/rust/driver.rs` builds against
  ...
  **Single-pass decode:** the driver calls the generated fallible
  `Probe::try_decode(&[u8]) -> Result<Probe, DecodeError>` (sofabgen 0.16.0,
  G-0001 fix) — `Ok`→`A <hex>`, `Err(DecodeError::Incomplete)`→`I`, else
  `R <class>`. The verdict arrives in **two** types since generator#463 adopted
  the §5.2.1 IStream contract (2026-09-03): `IStream::feed` answers
  `Result<Status, Error>`, because running out of bytes mid-field is an *outcome*
  and only the caller's framing can say whether more bytes are coming, so
  `sofab::Error` has no `Incomplete` variant at all; the two whole-buffer entry
  points (`try_decode`, `Decoder::finish`) **are** that framing and fold a
  trailing `Status::Incomplete` into the generated `DecodeError::Incomplete`,
  keeping it distinct from `DecodeError::Sofab(InvalidMsg)` — unfinished is not
  malformed. `build.sh`'s per-variant preamble imports `DecodeError` beside
  `Probe`, since it is generated code and not part of the corelib. This
...
- **cpp (cpp + c-cpp)** — one shared `drivers/cpp/driver.cpp` builds against BOTH
  ...
  **The receiver's field-span cap is a constructor argument** on the pure-C++
  corelib since corelib-cpp#128 (§6.2.1: no held limit, no default — the library
  has no number to invent), so every stream `driver.cpp` builds by hand must state
  it, and must state the *same* number the generated `try_decode` states or the
  chunked path would decode under a different policy than the one-shot path. The
  driver mirrors generated code exactly: `SOFAB_MAX_DYN_BUFFERED_FIELD` where the
  build configured receiver caps (limit mode), `SIZE_MAX` otherwise. The c-cpp
  wrapper takes no such argument; `build.sh` selects between the two constructor
  shapes with `-DCRUCIBLE_ISTREAM_TAKES_LIMITS`, set for `cpp` and `cpp-fixed`.
...
- **typescript** — `drivers/ts/driver.ts` runs on Node; `build.sh` bundles it +
  ...
  **`build.sh` type-checks before it bundles** (`tsc --noEmit --strict` over
  `driver.ts` + the generated `message.ts`, against the corelib source). esbuild
  strips types without checking them, so a corelib API change lands in the bundle
  intact and surfaces only when a record reaches the changed call — as a crash the
  comparator reports as a divergence on whatever input happens to be first.
  corelib-ts#161 caused exactly that twice: `new OStream()` lost its allocating
  form (the output buffer is the caller's, §6.6) and threw on record #0, and
  `FlushSink` became `(buffer, start, end)`, which the old one-argument sink would
  have read as "copy the whole window" — silently wrong bytes, not a crash. `tsc`
...
- **dart** — `drivers/dart/driver.dart` AOT-compiled with `dart compile exe`
  ...
  transitively, so `dart pub get` needs nothing hosted), then compiles. The
  dependency name must equal the `name:` in the corelib's own `pubspec.yaml`, which
  pub verifies: `sofa_buffers_corelib` — the organization slug plus `corelib` in
  pub.dev's `lowercase_with_underscores` form, the derivation every port follows,
  and byte-identical to corelib-zig's. It was `sofabuffers` until the registry
  rollout renamed it; generated code followed immediately, the hand-written driver
  did not. **Fallible,
  ...
  unresolved (PLAN §14).
- **kotlin (jvm + native)** — one shared `drivers/kotlin/driver.kt` builds against the
  SAME corelib compiled for two Kotlin Multiplatform targets; `build.sh <jvm|native>`
  selects which. Registered as two drivers (`kotlin-jvm`, `kotlin-native`) — one
  `commonMain` codec, but `Mem` is an `expect`/`actual` (byte-array `VarHandle`s on the
  JVM, indexed shifts natively), so they are two implementations of the same design and
  a divergence between them is a bug by construction. **The corelib is built by its own
  ...
  **Status-returning single-pass decode:** the generated
  `Probe.tryDecode(ByteArray, Probe) → DecodeStatus` fills the message and returns the
  §7 status — `INCOMPLETE`→`I`, `COMPLETE`→`A` — with a thrown `SofabException`→`R`,
  the class read off its `SofabError` (`ARGUMENT`/`BUFFER_FULL`/`INVALID_MSG`; that enum
  has no `USAGE`, §6.3 having abolished the category) and `LIMIT_EXCEEDED`→`L`.
  Scalars are Kotlin **unsigned** types (`UByte`/`UInt`/`ULong`, `UByteArray`/…), so the
  materialized walk prints unsigned decimal with no masking, and `Float` is a true
  32-bit float, so `toRawBits()` is the exact wire pattern — neither the Java widening
  care nor Dart's `Fp32Bits` companion channel applies here. On the `I` payload it sits
  in java's camp: it hands back the partial value it had already read (the soft
  `incomplete_value` axis), which is why adding it moved no camp.
  Materialize walker is **build-time generated** (`materialize_gen.py`, the rust/cpp/zig/dart
  camp): Kotlin has property reflection only on the JVM, and one walker has to serve
  both legs. Coverage engine is Jazzer on the JVM leg (`FuzzProbe.kt`, devcontainer —
  not compiled by `build.sh`, which builds only the replay driver); the native leg has
  none (PLAN §14).
