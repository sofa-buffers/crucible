...
> **Status: Phase 2 in progress** — the differential loop runs across eight
> drivers (C pacemaker, Go, Rust-std, Rust-no-std, C++, C++/c-cpp, Python-Cython,
> Python-pure) over the minimal `probe` schema. This document describes the
...
| Drivers: java/cs/ts/zig | planned | Phase 2 (remaining). |
...
(`80`, `ff ff ff`). Phase 2 grew it to a **5-accept vs 3-reject camp split** — the
C/C++/Rust lineage (c-cpp, cpp, c-cpp wrapper, rs, rs-no-std) accepts it as the
all-defaults message; two independent lineages, Go and Python (both cython and
pure), reject it. Real, hand-verified against all eight corelibs. Two
