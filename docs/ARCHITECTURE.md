...
| `scripts/check-catalog.py` | built | **The catalog gate**, driver-free and blocking. Two assertions. (1) The **state token** (✅/🔴/⚪) agrees across the index row, the write-up, and a `G-00NN`'s paired row — added 2026-08-03 after 46 write-ups were found contradicting the index or declaring nothing. (2) Since 2026-08-16, every closed finding carrying reproducers declares a **`**Guard:**`** — the gate corpus, sweep axis or oracle that re-checks it, or `none — <reason>`. The declaration is verified: a named corpus must hold either those bytes or a vector named for the finding (both promotion styles are legitimate; neither test alone covers both), a named axis must exist, a `none` must carry a reason. It exists because promotion is the step that silently does not happen — it falls weeks after the find, usually while several findings go green at once, and adding a converged vector to a green gate produces no visible change. 13 findings were closed but unguarded when the check was written. |
