...
| `scripts/materialize.sh` | built | Runs the materialized differential over the **full 12-driver roster** with `SOFAB_MATERIALIZE=1`, over `corpus/structured` — **75×12 → 0 divergences** (agreement) **+ a C-anchor conformance check** vs the reference (a family-wide-wrong dump is agreement-green, conformance-red). **A standing CI gate** (`replay.yml`). C is the schema-agnostic anchor (object-descriptor walk, `drivers/c/driver.c`); the other 11 hand-walk the decoded value with a schema-type table (a generated table would make them schema-agnostic too — the one open refinement). |
...
it (C via the object descriptor; the other 11 hand-walk the decoded value with a
schema-type table): **75×12 → 0 divergences**, all matching the
