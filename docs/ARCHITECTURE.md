...
| `drivers/common/CONTRACT.md` | built | Persistent length-prefixed protocol + canonical output. Since 2026-08-04 it additionally **specifies the two streaming axes** the generated API exposes and the replay protocol cannot otherwise reach — chunked decode (`SOFAB_SPLIT`, `SOFAB_CHUNK`, `SOFAB_CHUNK_SCRUB`) and streaming encode (`SOFAB_ENCODE`, `SOFAB_FLUSH`) — including the normative rule that the chunked verdict comes from the decoder's `status`, never from `finish()` (backends differ there: most throw, Dart returns null). Two `meta` keys, `chunked_decode` and `encode_surfaces`, record per backend what exists to be driven. **The specification is complete; no driver implements any of the five variables yet.** |
