...
> **Status: Phase 2 in progress** — the differential loop runs across nine
> drivers (C pacemaker, Go, Rust-std, Rust-no-std, C++, C++/c-cpp, Python-Cython,
> Python-pure, Java) over the minimal `probe` schema. This document describes the
...
| Drivers: cs/ts/zig | planned | Phase 2 (remaining). |
...
(`80`, `ff ff ff`). Phase 2 grew it to a **6-accept vs 3-reject camp split** — the
C/C++/Rust/Java camp (c-cpp, cpp, c-cpp wrapper, rs, rs-no-std, java) accepts it
as the all-defaults message; two independent lineages, Go and Python (both cython
and pure), reject it. Real, hand-verified against all nine corelibs. Two
independent implementations rejecting is stronger evidence than one outlier that
the lenient camp is wrong — exactly the pressure the PLAN §8 spec decision needs.
