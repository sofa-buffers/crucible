...
> **Status: Phase 2 in progress** — the differential loop runs across ten
> drivers (C pacemaker, Go, Rust-std, Rust-no-std, C++, C++/c-cpp, Python-Cython,
> Python-pure, Java, TypeScript) over the minimal `probe` schema. This describes
...
| Drivers: cs/zig | planned | Phase 2 (remaining). |
...
(`80`, `ff ff ff`). Phase 2 grew it to a **6-accept vs 4-reject camp split** — the
C/C++/Rust/Java camp (c-cpp, cpp, c-cpp wrapper, rs, rs-no-std, java) accepts it
as the all-defaults message; **three independent lineages — Go, Python (cython
and pure), and TypeScript — reject it**. Real, hand-verified against all ten
corelibs. Three unrelated implementations rejecting is strong evidence the
