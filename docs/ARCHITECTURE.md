...
> **Status: Phase 1 built** — a working two-language differential loop (C
> pacemaker + Go) over the minimal `probe` schema. This document describes the
...
| `scripts/run.sh` | built | Build all drivers → differential compare over a corpus. |
| `corpus/seeds/` | built | 6 agreeing seeds (the regression gate). |
...
| Drivers: rust/cpp/java/python/cs/ts/zig | planned | Phase 2. |
...
The Phase-1 loop found **F-0001** on its first run over 8 seeds:
`corelib-c-cpp` accepts a truncated trailing varint (`80`, `ff ff ff`) as the
all-defaults message; `corelib-go` rejects it as invalid. Real, hand-verified
against both corelibs — the differential oracle working as designed. Open pending
the PLAN §8 spec decision. See `results/FINDINGS.md` and
