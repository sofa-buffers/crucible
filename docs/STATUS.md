...
| G-0009 | generated C++ emits a schema-*unbounded* array as `std::array<T, 0>` (not `std::vector<T>`) | **open** — [generator#112](https://github.com/sofa-buffers/generator/issues/112) (sofabgen 0.16.0, crucible#10) — silently drops every element of an *accepted* array (value divergence: `A []` vs family's real elements). The `max_dyn_array_count` cap still fires (over-cap → `L`, agrees). Only the array path; C++ string/blob caps are fine. `cpp` held out of the array dimension of limit mode (`scripts/run-limits.sh`) until fixed. Repro: `03 03 07 08 09` → family `[7,8,9]`, C++ `[]`. See docs/SOFABGEN.md |
