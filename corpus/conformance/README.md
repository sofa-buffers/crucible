...
| `b_array_trailing_defaults_kept.bin` | **§3 as amended by [documentation#31](https://github.com/sofa-buffers/documentation/pull/31)** — `count` is a **capacity**, so the wire count *is* the array's length: a trailing default run is part of the value and MUST be preserved. Neither trimming nor padding is allowed. | `arrays.u8` = `[1,2,3,0,0]` (count 5, two trailing defaults) → all 13 accept and re-encode to **itself**, `a606 0305010203 0000 07`, byte-identical. A driver that trims to count 3 — the pre-#31 F-0010 rule — diverges. |
| `b_array_canonical_ctl.bin` | control | the **different** value `[1,2,3]` (count 3), `a606 0303010203 07`, 8 bytes. Under #31 this is not the canonical form of `b` — the two are distinct values, and each round-trips to itself. That is exactly what makes the pair a test: if any driver still trims, the two collapse onto one wire form. (Until 2026-07-28 this file carried the four empty frames of the pre-POC canonical form; regenerated from `gen.py`.) |
