...
| [`results/FINDINGS.md`](results/FINDINGS.md) | **the** findings **index** — one row per entry (F-00NN, and codegen defects G-00NN), carrying only the link, the upstream ticket and the state. **Generated** from the write-ups by `scripts/gen-findings.py`; edit the write-up, not this file. |
| `findings/<id>/NOTES.md` | **the** write-up for one finding, and the single owner of everything about it: what it is, its reproducer, the attribution, and its resolution. Every entry in the index has one. |
...
|---|---|---|
| `count`, `maxlen`, `N`, a field's declared type — **schema** facts | only **generated code** (the corelib is schema-agnostic *by design*) | `generator` (sofabgen); log it as a `G-00NN` codegen defect — a folder under `findings/`, indexed in [`results/FINDINGS.md`](results/FINDINGS.md) |
...
- **Generated decode may not surface the verdict.** The Rust `Probe::decode` is
  ...
  corelib's real result and log the codegen gap as a `G-00NN` finding folder.
