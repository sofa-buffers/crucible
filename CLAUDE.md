...
| [`docs/CI.md`](docs/CI.md) | the three GitHub-Actions workflows (image / replay / nightly) — which gates block on every push/PR and which run nightly. |
...
| [`results/FINDINGS.md`](results/FINDINGS.md) | the triaged-findings index (F-00NN): title, impls, axis, status, and links to each `findings/<id>/NOTES.md` reproducer. |
| [`results/CLUSTERS.md`](results/CLUSTERS.md) | root-cause clusters — raw divergences grouped by camp partition (generated by `oracle/cluster.py`), so thousands of rows collapse to a handful of causes. |
| [`README.md`](README.md) | the outward/public face — pitch, how-it-works, and the suite index; the human front door, not an agent source of truth. |
