...
# footgun run-limits.sh has). **BLOCKING since 2026-08-16.** It was report-only per the
# project's own rule — an axis blocks once it is green or every divergence it surfaces is
# catalogued — and it has been green for weeks while the promotion step, which the comment
# here called "a follow-up", was never taken. Three other axes were promoted the day they
# went green; this one was forgotten, and a test that cannot fail is not a test.
echo "==> [sweep] union pass (blocking): rebuilding drivers against probe-union" >&2
SCHEMA="$ROOT/schema/probe-union.sofab.yaml" CORPUS="$ROOT/corpus/union" "$ROOT/scripts/run.sh" >/dev/null
echo "==> [sweep] union axes: wiretype §7.3 + repeated-id §7.4 + over-bound §7.1 + reserved-subtype §4.6 + truncation §7" >&2
python3 "$SWEEP" --union
echo "==> [sweep] rebuilding drivers back to probe (restore the default binary state)" >&2
CORPUS="$ROOT/corpus/seeds" "$ROOT/scripts/run.sh" >/dev/null
echo "==> [sweep] done (every axis blocking, probe and union)" >&2
