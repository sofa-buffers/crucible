...
| [F-0008](../findings/F-0008-cpp-c-cpp-nested-seq-in-string-array-hang/NOTES.md) | corelib-c-cpp **hangs (infinite loop / DoS)** on a nested `SEQUENCE_START` inside the `string_array` field — a 4-byte input (`c6 0c c6 07`) | corelib-c-cpp (C++ object-stream path) | liveness / DoS | **open** — filed [corelib-c-cpp#84](https://github.com/sofa-buffers/corelib-c-cpp/issues/84). Found by the **structure-aware mutator** + localized by the comparator **per-driver timeout**. `c` (same `istream.c`), `cpp`, `go`, `rust` all return `I` instantly; only the c-cpp C++ read-sequence path spins. Liveness analogue of F-0003 |
