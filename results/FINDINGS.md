...
| [F-0055](../findings/F-0055-nostd-visitor-scope-stack-overflow-discarded/NOTES.md) | **the no-std visitor's scope stack holds 8 entries and its overflow is discarded — a field written after the unwind is *silently lost*** — `stack: heapless::Vec<_Loc, 8>` with `let _ = self.stack.push(self.cur)`. `MAX_DEPTH` is **255** (§6.2), so 9 levels is legal; past eight the push does nothing, the matching `pop` restores the wrong scope, and a later field binds nowhere. 24-byte isolate: 12 impls re-encode `nested.f32 = 1.5`, **rust-no-std accepts and returns the empty message**. No error, no rejection — the worst of the three outcomes | generator (sofabgen **no-std backend**) — **G-0035** | accept_value | 🔴 **open — found 2026-08-03** while triaging the two large rust-nostd camps, reached by reading the generated source after four black-box hypotheses were refuted. **Threshold is exactly the capacity**: `nested` + 8 inner levels = 9 pushes against a stack of 8; 7 inner levels is unanimous. **Why F-0050's depth vectors missed it:** nesting *only* unknown sequences is unaffected to depth 14 — every scope is `Dead`, the dropped pushes are the deepest, and the surplus pops return `unwrap_or(Root)`, which is accidentally correct. The corruption needs a **real scope underneath the overflow**. Attribution is codegen: the stack, its size and the discarded `Result` are all generated, and **rust-std is correct** — same schema, same generator, growable `Vec`. Fix: size to `MAX_DEPTH`, and/or set the existing `err` flag instead of discarding the overflow. **Filed 2026-08-03 as [generator#283](https://github.com/sofa-buffers/generator/issues/283).** |
...
| G-0035 (= F-0055) | [generator#283](https://github.com/sofa-buffers/generator/issues/283) | 🟡 **fix landed upstream, verification pending** — issue closed 2026-08-03 by `bd67d2b` ("stack only live scopes, so a deep skip can't lose"), but its generator CI run was still building, so no sofabgen artifact carries it yet. **F-0055** above stays open until the artifact exists and the 24-byte isolate is re-measured. |
