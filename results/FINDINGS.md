...
| G-0020 (= F-0035) | not yet filed | 🔴 **open** — struct-array element decode appends id-blind in 10 backends (leaf elements place by id, struct elements `append`); §5.1 id-is-index violated, values corrupted on gaps/reopens. Found 2026-07-27 on the poc family. Crucible finding **F-0035** |
| G-0021 (= F-0036) | not yet filed | 🔴 **open** — generated marshals never trim the trailing all-default sequence-element run (§3/§5.1) nor omit the `M = 0` wrapper (POC §2); 12 of 13 non-canonical on re-encode, only `c` normalizes. Crucible finding **F-0036** |
| G-0022 (= F-0037) | not yet filed | 🔴 **open** — the C++ backend's element decode leaves a phantom default element after a §7.3 mistyped-skip inside `struct_array` (`cpp` + `cpp-c-cpp`). Crucible finding **F-0037** |
