...
| [F-0054](../findings/F-0054-id-max-not-checked-on-sequence-end-header/NOTES.md) | **the `ID_MAX` ceiling is not applied to a *sequence-end* header's id** — `76 87 80 80 80 40` (6 B): a skipped unknown sequence closed by an end marker whose header id is **2³¹**, one past `ID_MAX`. Nine reject, four accept and re-encode to the empty message. At exactly `ID_MAX` all 13 agree, so the ceiling itself is right everywhere — what is missing is the check **on wire type 7**. The same over-ceiling id on an *unsigned* header is rejected by all 13 | **corelib-go, corelib-py, corelib-ts** | verdict | 🔴 **open — found 2026-08-03**, same corpus review. Cluster of 4, minimized 56 B → 31 B → rebuilt as a 6-byte isolate. §6.2 lists `ID_MAX` among the **format-wide ceilings**, *"identical for every implementation"*, and §5.2 makes exceeding one malformed **regardless of what follows**; nothing carves out the end marker, which is an ordinary header whose wire type happens to be 7. Its id is never *used* — presumably why the check was skipped — but unused is not unvalidated. py-cython and py-pure are both affected, consistent with one shared `corelib-py` reader. Reachable with no valid schema context at all, inside a skipped subtree. **Filed 2026-08-03, one issue per impl: [corelib-go#67](https://github.com/sofa-buffers/corelib-go/issues/67), [corelib-py#58](https://github.com/sofa-buffers/corelib-py/issues/58), [corelib-ts#83](https://github.com/sofa-buffers/corelib-ts/issues/83).** |
