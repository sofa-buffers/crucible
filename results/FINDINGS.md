...
| [F-0052](../findings/F-0052-cpp-array-element-width-bound-never-armed/NOTES.md) | **the cpp backend never arms `readArray`'s element-width bound, so `cpp` masks an over-width array element** — `arrays.u8` with an element of 5208 is `R invalid_msg` on 12 impls; **cpp accepts and re-encodes it as 88** (5208 mod 256), the mask-and-keep that documentation#32 forbids. Reproduced on `u8`, `i8` and `u16`. The same over-width value at a **scalar** position is rejected by all 13 including cpp, so F-0033 is genuinely closed there and only the array-element path is not | generator (sofabgen **C++ backend**) — **G-0034** | verdict / accept_value | 🔴 **open — found 2026-08-03** by the 4-hour pacemaker round (577 M execs, corpus 878 → 2609): its one new cluster, minimized 49 B → 11 B. **corelib-cpp is not at fault** — it shipped the check in [corelib-cpp#67](https://github.com/sofa-buffers/corelib-cpp/issues/67) (`c4b8eef`, 290 lines), but as an **opt-in** parameter: `readArray(T&, long schemaCount, long dynCap, ElemBound elem = {})` guards the bounded decode behind `if (elem.armed)`. All ten generated array reads pass **two** arguments (`readArray(u8, 5)`) and `grep -c ElemBound` over the generated header is **0**, so the bound is never armed and the unbounded path masks. The **F-0042 shape**: corelib widens a hook, the fix completes only when the backend consumes it. Corroborated by `cpp-c-cpp` being correct — same C++ backend, different corelib API. **Why no gate caught it:** the over-width axis had vectors only at *scalar* positions (F-0033's four), the same scalar-only blind spot F-0049 had for fp32 raw bits. **Gate gap closed 2026-08-03:** `sweep_overbound` now derives the declared element width from the schema and sweeps it at every integer array position (49 → 67 vectors, over/at/under/at-min). It catches exactly this finding — 9 divergences, all cpp — and is carved out behind `_F0052_CARVEOUT` only because the axis is blocking; a one-line deletion when #279 lands. **Filed 2026-08-03 as [generator#279](https://github.com/sofa-buffers/generator/issues/279)** — the fix is one argument per call site, and corelib-cpp already ships the helper (`ElemBound::of<E>()`, documented as safe to hand in unconditionally since it degrades to unarmed for 64-bit). |
| [F-0053](../findings/F-0053-array-count-vs-remaining-bytes-precedes-varint-check/NOTES.md) | **an array `count` larger than the remaining bytes short-circuits to `INCOMPLETE` before the element varint is validated** — `43 0b 80×10` (12 B): an unknown-id array declaring 11 elements over ten all-continuation bytes. The element varint cannot terminate before its eleventh byte, so it is over 64 bits and `INVALID` (§4.1), and §5.2 says that dominates the truncation. The **threshold is exactly at count 11**, the first count that outruns the ten bytes on hand — so go and typescript never look at the varint; an availability pre-check returns first | **corelib-go, corelib-ts** | verdict | 🔴 **open — found 2026-08-03** in the first review of the nightly's accumulated corpus (8512 inputs). Cluster of 50, minimized 25 B → 12 B. Controls: count 10 with the identical payload is unanimous `R`, and count 11 with eleven legal one-byte elements is unanimous — it is their *relation*, not either alone. **Only reachable where no schema bound applies** (an unknown id, or a §7.3-mistyped one): at a declared position the schema `count` fires first and masks it, which a control confirms. Attribution is corelib — skipping an unknown field is pure wire mechanics with no schema knowledge. Two corelibs sharing one defect is coherent: *bail out early if the declared count cannot fit* is an obvious optimisation, wrong only because §5.2 fixes a precedence it inverts. Class of F-0007/F-0012/F-0014/F-0043, but the loser here is a **format** rule (§4.1's 64-bit ceiling), not a schema bound. **Filed 2026-08-03, one issue per impl: [corelib-go#66](https://github.com/sofa-buffers/corelib-go/issues/66), [corelib-ts#82](https://github.com/sofa-buffers/corelib-ts/issues/82).** |
| [F-0054](../findings/F-0054-over-idmax-id-on-sequence-end-wrongly-rejected/NOTES.md) | **an over-`ID_MAX` id on a *sequence-end* header is wrongly rejected** — `76 87 80 80 80 40` (6 B): a skipped unknown sequence closed by an end marker whose header id is **2³¹**, one past `ID_MAX`. Four accept and re-encode to the empty message (**correct**); nine reject. At `ID_MAX` and below all 13 accept, so the nine hold the controls only by staying under a bound that does not apply to wire type 7. The same over-ceiling id on an *unsigned* header is rejected by all 13 — and that is right | **corelib-c-cpp, corelib-cpp, corelib-cs, corelib-dart, corelib-java, corelib-rs, corelib-rs-no-std, corelib-zig** (9 drivers, 8 repos) | verdict | 🔴 **open — found 2026-08-03, attribution INVERTED the same day.** Cluster of 4, minimized 56 B → 31 B → rebuilt as a 6-byte isolate. **Spec basis is the merged `CORELIB_PLAN.md` @ `main@51c777d` and nothing else:** §4.9 — a decoder *"**MUST accept** a sequence-end header (wire type `0b111`) carrying **any** id, **discard** that id, and re-encode the marker as `0x07`"*, a non-zero id being *"**not** `INVALID`"* but normalized away as a non-minimal varint is (§4.1); §6.2 — `ID_MAX` bounds *"the id of a **value-bearing** field header"* and does **not** apply to the end marker; §5.2/§6.3 do not list this among the `INVALID` conditions. Only §4.1's 64-bit varint bound still binds. Fix is a **removal**: skip the `ID_MAX` guard on wire type 7. §7.2's new tolerance class **5b** names this case verbatim. **Originally filed the other way round against the four accepters ([corelib-go#67](https://github.com/sofa-buffers/corelib-go/issues/67), [corelib-py#58](https://github.com/sofa-buffers/corelib-py/issues/58), [corelib-ts#83](https://github.com/sofa-buffers/corelib-ts/issues/83)); all three were closed on a proposed rule that never became normative — those closures are stale and must not be acted on.** **Re-filed 2026-08-03 against the nine, one issue per repo: [corelib-c-cpp#128](https://github.com/sofa-buffers/corelib-c-cpp/issues/128), [corelib-cpp#68](https://github.com/sofa-buffers/corelib-cpp/issues/68), [corelib-cs#51](https://github.com/sofa-buffers/corelib-cs/issues/51), [corelib-dart#30](https://github.com/sofa-buffers/corelib-dart/issues/30), [corelib-java#60](https://github.com/sofa-buffers/corelib-java/issues/60), [corelib-rs#45](https://github.com/sofa-buffers/corelib-rs/issues/45), [corelib-rs-no-std#66](https://github.com/sofa-buffers/corelib-rs-no-std/issues/66), [corelib-zig#33](https://github.com/sofa-buffers/corelib-zig/issues/33)** — each carries the decode-side `file:line` and the note that the encoder-side `ID_MAX` checks stay. Verdicts re-measured against every corelib's current `main` on 2026-08-03: unchanged, 4 accept / 9 reject, all three controls unanimous. |
...
| G-0035 (= F-0055) | [generator#283](https://github.com/sofa-buffers/generator/issues/283) | 🔴 **open — found 2026-08-03.** The no-std backend emits the visitor's scope stack as `heapless::Vec<_Loc, 8>` and pushes with `let _ = self.stack.push(self.cur)`, discarding the overflow `Result`. `MAX_DEPTH` is 255, so the capacity is two orders too small; and because the error is dropped, exceeding it corrupts the scope silently rather than failing. The std backend uses a growable `Vec` and is correct. Crucible finding **F-0055** |
