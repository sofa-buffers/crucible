...
| [F-0052](../findings/F-0052-cpp-array-element-width-bound-never-armed/NOTES.md) | **the cpp backend never arms `readArray`'s element-width bound, so `cpp` masks an over-width array element** — `arrays.u8` with an element of 5208 is `R invalid_msg` on 12 impls; **cpp accepts and re-encodes it as 88** (5208 mod 256), the mask-and-keep that documentation#32 forbids. Reproduced on `u8`, `i8` and `u16`. The same over-width value at a **scalar** position is rejected by all 13 including cpp, so F-0033 is genuinely closed there and only the array-element path is not | generator (sofabgen **C++ backend**) — **G-0034** | verdict / accept_value | ✅ **resolved — found and fixed 2026-08-03.** Found by the 4-hour pacemaker round (577 M execs, corpus 878 → 2609): its one new cluster, minimized 49 B → 11 B. The C++ backend never armed `readArray`'s `ElemBound`, so corelib-cpp#67's check never ran and `cpp` alone **masked** the over-width element (5208 → 88) instead of rejecting — and on the truncated form degraded to `I`, which was the second symptom and a separate camp. **Fixed via [generator#279](https://github.com/sofa-buffers/generator/issues/279) (closed).** Verified 2026-08-03 on sofabgen `0.0.0-20260803154628-1e4359a8a1c0`: all 6 reproducers produce **no cluster**. Checked on the **value**, not on agreement — the three `*_elem_over` forms and the truncated one are `R invalid_msg` on all 13, and the control `ctl_u8_array_inrange` still re-encodes `…c801…`, i.e. 200 is **preserved, not masked**. Both oracles: round-trip clean and `materialize.sh` 108 × 13 with 0 divergences and 0/108 C-anchor mismatches. Reproducers in the green `corpus/regression/` gate (`F0052_*`), controls included; both camp signatures deleted from `known-clusters.txt` so a regression reports as **NEW**. |
| [F-0053](../findings/F-0053-array-count-vs-remaining-bytes-precedes-varint-check/NOTES.md) | **an array `count` larger than the remaining bytes short-circuits to `INCOMPLETE` before the element varint is validated** — `43 0b 80×10` (12 B): an unknown-id array declaring 11 elements over ten all-continuation bytes. The element varint cannot terminate before its eleventh byte, so it is over 64 bits and `INVALID` (§4.1), and §5.2 says that dominates the truncation. The **threshold is exactly at count 11**, the first count that outruns the ten bytes on hand — so go and typescript never look at the varint; an availability pre-check returns first | **corelib-go, corelib-ts** | verdict | ✅ **resolved — found and fixed 2026-08-03.** Found in the first review of CI's accumulated 8512-input corpus. Cluster of 50, minimized 25 B → 12 B; threshold measured exactly at count 11. An array `count` outrunning the remaining bytes short-circuited to `INCOMPLETE` **before** the element varint was validated, so an overlong varint inside such an array was reported `I` instead of `R` — §5.2 precedence, `INVALID` wins over `INCOMPLETE`. **Fixed: [corelib-go#66](https://github.com/sofa-buffers/corelib-go/issues/66) and [corelib-ts#82](https://github.com/sofa-buffers/corelib-ts/issues/82) (both closed).** Verified 2026-08-03 against both corelibs' current `main`: all 5 reproducers produce **no cluster**, `r1_count11_overlong_elem` is `R invalid_msg` on all 13 where go and typescript said `I`, and the control `ctl_count11_enough_bytes` still accepts — read out per driver, since a family-wide over-rejection would look like the same unanimity. Both oracles green. Reproducers in `corpus/regression/` (`F0053_*`), controls included; the camp is deleted from `known-clusters.txt`. |
| [F-0054](../findings/F-0054-idmax-on-sequence-end-header/NOTES.md) | **the `ID_MAX` ceiling and a *sequence-end* header's id** — `76 87 80 80 80 40` (6 B): a skipped unknown sequence closed by an end marker whose header id is **2³¹**, one past `ID_MAX`. Four accepted it, nine rejected; at `ID_MAX` and below all 13 accepted, so **exactly one test point** was ever in dispute | **corelib-go, corelib-py, corelib-ts** (4 drivers, 3 repos) | verdict | ✅ **resolved — found, specified and fixed 2026-08-03**, after the attribution moved twice (see the finding's History). Spec: **[documentation#35](https://github.com/sofa-buffers/documentation/pull/35) merged as `main@acd27a4`**, reverting #34 — §4.9 *"discarded is not unvalidated"*: a sequence-end id is discarded but bounded by `ID_MAX` like every other header's, §6.2 binds every header without exception, and §5.2/§6.3 now name an over-ceiling **id**, which they never did. The bound is on the id's *value*, not its spelling, so §4.1 is untouched — that is what separates it from the abandoned Option C. Fixed same day: **[corelib-go#70](https://github.com/sofa-buffers/corelib-go/pull/70), [corelib-py#60](https://github.com/sofa-buffers/corelib-py/pull/60), [corelib-ts#86](https://github.com/sofa-buffers/corelib-ts/pull/86)** (issues go#69, py#59, ts#85). go's fix deleted its `t != TypeSequenceEnd` exception, which also removed a **self-contradiction between its two decode surfaces** (`cursor.go` had the exception, `decoder.go` did not) — no separate finding needed. **Verified by verdict, not by agreement:** the isolate is `R invalid_msg` on all 13 and the three controls (id 0, 3, `ID_MAX`) are `A` on all 13, read out per driver. A family-wide over-tightening onto Option C would have shown as the same unanimous "0 divergences". All five inputs are in the green `corpus/regression/` gate (`F0054_*`), controls included, and the camp is removed from `known-clusters.txt` so a regression reports as **NEW**. |
| [F-0055](../findings/F-0055-nostd-visitor-scope-stack-overflow-discarded/NOTES.md) | **the no-std visitor's scope stack holds 8 entries and its overflow is discarded — a field written after the unwind is *silently lost*** — `stack: heapless::Vec<_Loc, 8>` with `let _ = self.stack.push(self.cur)`. `MAX_DEPTH` is **255** (§6.2), so 9 levels is legal; past eight the push does nothing, the matching `pop` restores the wrong scope, and a later field binds nowhere. 24-byte isolate: 12 impls re-encode `nested.f32 = 1.5`, **rust-no-std accepts and returns the empty message**. No error, no rejection — the worst of the three outcomes | generator (sofabgen **no-std backend**) — **G-0035** | accept_value | ✅ **resolved — found and fixed 2026-08-03.** Found while triaging the two large rust-nostd camps, reached by reading the generated source after four black-box hypotheses were refuted. The no-std visitor's scope stack was `heapless::Vec<_Loc, 8>` with `let _ = self.stack.push(self.cur)` — `MAX_DEPTH` is **255** (§6.2), so past eight the push did nothing, the matching `pop` restored the wrong scope, and a field written after the unwind bound nowhere: rust-no-std **accepted and returned the empty message**, the worst of the three outcomes. **Fixed by [generator#283](https://github.com/sofa-buffers/generator/issues/283)** (`bd67d2b`, *"stack only live scopes, so a deep skip can't lose"*). Verified 2026-08-03 on sofabgen `0.0.0-20260803165303-bd67d2b2f84c` — the first artifact carrying it; the previous build was 67 minutes older, which is why an earlier sweep still showed the camp. **Checked on the value, and here that is the whole point:** all 5 reproducers agree, and rust-no-std now re-encodes `5602200000c03f07` (`nested.f32 = 1.5`) instead of the empty message, with `r3_depth9_wrapper_lost` at `c60c020a4107` — read out per driver, because a silent-loss defect where *every* impl returned empty would present as the same unanimous agreement. `materialize.sh` 108 × 13, 0 divergences, 0/108 C-anchor mismatches. Reproducers in the green `corpus/regression/` gate (`F0055_*`), depth-8 and no-nesting controls included; the camp is deleted from `known-clusters.txt`. |
...
| G-0007 (= F-0003) | [generator#78](https://github.com/sofa-buffers/generator/issues/78) | ✅ **resolved** in sofabgen 0.15.1 — detail and verification in the **G-0007** section below. |
...
| G-0026 (= F-0033) | [generator#266](https://github.com/sofa-buffers/generator/issues/266) | ✅ **resolved** — issue closed; verified under **F-0033** above. |
| G-0027 (= F-0043) | [generator#267](https://github.com/sofa-buffers/generator/issues/267) | 🔴 **open** — the only generator ticket still open; detail and current measurement under **F-0043** above. |
| G-0028 (= F-0044) | [generator#268](https://github.com/sofa-buffers/generator/issues/268) | ✅ **resolved** — issue closed 2026-08-02; verified under **F-0044** above. |
| G-0029 (= F-0045) | [generator#270](https://github.com/sofa-buffers/generator/issues/270) | ✅ **resolved** — issue closed 2026-08-02; verified under **F-0045** above. |
| G-0030 (= F-0046) | [generator#271](https://github.com/sofa-buffers/generator/issues/271) | ✅ **resolved** — issue closed 2026-08-02; verified under **F-0046** above. |
| G-0031 (= F-0047) | [generator#272](https://github.com/sofa-buffers/generator/issues/272) | ✅ **resolved** — issue closed 2026-08-02; verified under **F-0047** above. |
| G-0032 (= F-0048) | [generator#273](https://github.com/sofa-buffers/generator/issues/273) | ✅ **resolved** — issue closed 2026-08-02; verified under **F-0048** above. |
| G-0033 (= F-0049) | [generator#275](https://github.com/sofa-buffers/generator/issues/275) | ✅ **resolved** — issue closed 2026-08-02; verified under **F-0049** above. |
| G-0034 (= F-0052) | [generator#279](https://github.com/sofa-buffers/generator/issues/279) | ✅ **resolved** — issue closed, fixed by [generator#281](https://github.com/sofa-buffers/generator/pull/281) (`0f80a53`, "arm readArray's element-width bound"); verified under **F-0052** above. |
| G-0035 (= F-0055) | [generator#283](https://github.com/sofa-buffers/generator/issues/283) | ✅ **resolved** — issue closed 2026-08-03, fixed by `bd67d2b`; verified under **F-0055** above on the first sofabgen artifact carrying it. |
