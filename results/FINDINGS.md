...
| [F-0031](../findings/F-0031-fp32-snan-quieted-py-cython-ts-dart/NOTES.md) | **an fp32 signaling NaN (0x7F800001) is quieted to 0x7FC00001** on decode→re-encode (and in the materialized raw-bits walk) by py-cython / typescript / dart, where the other 10 (incl. py-pure) preserve it — §4.6 requires bit-for-bit float round-trip, no normalization | corelib-py (Cython engine) + corelib-ts + corelib-dart; **corelib-only, not codegen** | accept_value (round-trip + materialized) | 🟡 **PARTIALLY RESOLVED — down to one impl.** **Re-verified 2026-07-29** against corelibs **0.9.0 @ main** + sofabgen **0.21.0** — the first family carrying the merged sparse-array rewrite (documentation#29/#31). **Only `typescript` still quiets the sNaN**; the other 12 preserve it bit-for-bit. The codegen residual [generator#235](https://github.com/sofa-buffers/generator/issues/235) is **still open**; [generator#251](https://github.com/sofa-buffers/generator/pull/251) ("bit-exact fp32 (#235)") was **closed unmerged**, so no fix has landed yet. *History (re-verified 2026-07-25):* — py ([corelib-py#49](https://github.com/sofa-buffers/corelib-py/issues/49)), **dart** ([corelib-dart#15](https://github.com/sofa-buffers/corelib-dart/issues/15) + generator#226) and the corelib-ts raw channel ([corelib-ts#66](https://github.com/sofa-buffers/corelib-ts/issues/66)/#72, `a6f31d6` — added `Cursor.readFp32Raw`) are fixed. **ts still quiets:** the generated ts code decodes via `readFp32()`/stores a JS `number`/re-emits `writeFp32`, never using the raw channel → a **codegen residual, [generator#235](https://github.com/sofa-buffers/generator/issues/235)** (the ts analogue of dart's generator#226). Re-verified 2026-07-25: only `typescript` diverges. **Scope:** only fp32 *signaling* NaN — the quiet-payload NaN (0x7FC00001), negative NaN, and fp64 sNaN all round-trip on 13. 0x7F800001 differs from a quiet NaN only in bit 22 (the is-quiet bit); the 3 double-backed fp32 impls set it (classic sNaN-quieting on load into a 64-bit double). **py-pure preserves it** → the sibling split pinning it to the double-backed fp32 path, not all-Python. **Root cause:** the fp32 payload is widened to a double before re-emit/materialize; the corelib must carry raw 32-bit bits (§4.6 'never inspects or normalizes'). **Found 2026-07-23 by the WP-06 float-specials vectors** — the first suite carrying an fp32 sNaN vector (canonical.md:107-109 had flagged it as a known limit). Carved out of the green corpus/structured gate until fixed. |
...
| [F-0038](../findings/F-0038-skipped-string-utf8-validated/NOTES.md) | **six corelibs UTF-8-validate a string field they are *skipping*** — a 3-byte isolate (`4a 0a 8a`: unknown id 9, fixlen subtype string, payload `0x8a`) is `A` on 7 and `R invalid_msg` on 6. CORELIB_PLAN §6.4 is normative and explicit: *"Skipped fields are never validated … UTF-8 validation runs only where a `string` is **materialized** … never on skip, in any mode"* — so the rejecters violate a MUST | **codegen for four, corelib for two** (corrected 2026-07-29 — the original "corelib-only" attribution was wrong). generated visitor validates before resolving the destination: rust-std, rust-no-std, java, csharp → **generator** (**G-0024**). corelib hands the visitor a finished language string: corelib-go, corelib-dart → those two repos, but each needs the codegen half too | verdict | 🔴 **OPEN — [generator#257](https://github.com/sofa-buffers/generator/issues/257) (G-0024) + [corelib-go#57](https://github.com/sofa-buffers/corelib-go/issues/57) + [corelib-dart#22](https://github.com/sofa-buffers/corelib-dart/issues/22).** **Attribution corrected 2026-07-29:** four of the six issues filed that morning (corelib-rs#39, -rs-no-std#59, -java#52, -cs#44) were **misfiled and are closed** — those corelibs carry no UTF-8 code on the decode path at all, proven statically *and* by feeding the isolates through the bare corelib with a no-op visitor, which already returns the required outcome. The generated visitor materializes and validates *before* it resolves the destination, so an unmatched id falls through the switch with the sticky invalid flag already set; the correct shape already exists in the same generator for **zig** (destination switch first, `utf8_valid` inside the matched arm), which is why zig passes. `go` and `dart` are genuinely corelib-side — their decoders hand the visitor a finished language string — and their fixes are **two-half**: [corelib-go#59](https://github.com/sofa-buffers/corelib-go/pull/59) and [corelib-dart#24](https://github.com/sofa-buffers/corelib-dart/pull/24) are open as drafts and **must not merge alone** (measured: go then accepts invalid UTF-8 in a declared field and breaks the §5.2 anti-folding control; dart's branch is inert). Landing order: generator#257 Class A → the go/dart backends → the two corelib PRs. **Re-verified 2026-07-29** against corelibs **0.9.0 @ main** + sofabgen **0.21.0** — the first family carrying the merged sparse-array rewrite (documentation#29/#31). Still reproduces on both isolates. Three controls isolate the axis: same id with **valid** UTF-8 → all 13 `A` (not the unknown id); same byte with subtype **blob** → all 13 `A` (only `string` triggers it); the same byte in a **declared** field → all 13 `R` (the check itself is right, its *placement* is wrong). A §7.3 variant (`d6 0c 02 12 ff ff 07`, fixlen string at a `struct_array` element slot) splits the same six, so unknown-id and mistyped-skip share the root cause. Neighbour precedent **F-0012**: corelib-ts validating the *fixlen word* while skipping was the correct half — framing must be checked on skip, **content** must not. **Found 2026-07-27 by the 60-min pacemaker round** (105.6 M execs, corpus 890 → 1121); clusters 6 + 13 of the post-fuzz triage. |
| [F-0039](../findings/F-0039-mistyped-array-allocates-declared-field/NOTES.md) | **a §7.3-mistyped array is allocated into the declared field** — an `ARRAY_SIGNED` header at the `array of u8` slot (`a6 06 04 01 06 07`) must be skipped, but java/csharp resize the destination from the skipped header's count and re-encode a 1-element array of zeros (`a6 06 03 01 00 07`) where 11 drivers emit nothing | generator (**java + csharp backends**, codegen, **G-0023**) — the corelibs hand both a correctly parsed `(id, kind, count)`; `c`/`cpp-c-cpp` on the same corelibs are correct | accept_value | 🔴 **OPEN — filed [generator#254](https://github.com/sofa-buffers/generator/issues/254)**. §7.3: *"MUST NOT decode its payload into the declared field"* — sizing the destination changes the value from `[]` to `[0]`. Two controls isolate it: a well-typed array round-trips on all 13, and a mistyped **scalar** is skipped by all 13, so only the array kind leaks. Same block also applies the schema `count` bound before the kind check, which §7.3 inverts. **Found 2026-07-29** by `wiretype_sweep` (30 of 332 vectors) |
| [F-0040](../findings/F-0040-overlong-varint-deferred-to-incomplete/NOTES.md) | **an already-overlong varint is reported `INCOMPLETE` instead of `INVALID`** — ten varint bytes that all set the continuation flag cannot terminate within the 10 bytes §4.1 allows, yet corelib-c-cpp asks for more input | corelib-c-cpp (`src/istream.c`, `_varint_decode`) — `c` + `cpp-c-cpp`; `cpp` (same generated C++ over corelib-cpp) rejects with the family, which pins it to the corelib | verdict | 🔴 **OPEN — filed [corelib-c-cpp#116](https://github.com/sofa-buffers/corelib-c-cpp/issues/116)**. §4.1 (INVALID iff longer than 10 bytes) + §5.2 precedence (malformed *regardless of what follows*). The width guard is evaluated on **entry for the next byte**, so `varint_shift` reaches 70 and the verdict waits for an 11th byte that never comes. Controls: a legal non-minimal 10-byte varint → all 13 `A`; an 11-byte varint → all 13 `R`, so neither the rule nor the width is wrong, only the boundary. **Found 2026-07-29**, cluster 6 |
| [F-0041](../findings/F-0041-overindex-reject-precedes-7-3-skip/NOTES.md) | **the over-index reject is applied before the §7.3 skip** — a wrapper element that is both past `count` and mistyped (`c6 0c 40 01 07`) is rejected on its id, where §7.3 requires it skipped first, after which no element index exists | corelib-c-cpp (`src/object.c`, descriptor path → `c`) **and** corelib-cpp (`sofab.hpp`, `cap` applied at the element header → `cpp`) — two proximate causes, one shape; `cpp-c-cpp` is unaffected | verdict | 🔴 **OPEN — filed [corelib-c-cpp#117](https://github.com/sofa-buffers/corelib-c-cpp/issues/117) + [corelib-cpp#58](https://github.com/sofa-buffers/corelib-cpp/issues/58)**. §7.3: *"Against a schema bound, this clause wins … the schema bound applied only to a field that survives it"*; §7.4 says the same from the other side. Each rule alone is unanimous — over-index **well-typed** → all 13 `R`, in-range **mistyped** → all 13 `A` — so this is purely clause ordering. The ordering needs **no ruling**: §7.3's rule is unconditional (its precondition is the wire type, not the id), "Against a schema bound, this clause wins" is the rule rather than the fixlen example that derives it, §7.4 already applies the same principle (*"An occurrence skipped under §7.3 is **not** an occurrence for this clause"*), and CORELIB_PLAN §4.8 gives the reason in general terms (*"the field was never this array's value"*). An earlier caveat suggesting the clause might be scoped to the fixlen count word was **withdrawn 2026-07-29** on both issues. **Found 2026-07-29**, cluster 3 |
| [F-0042](../findings/F-0042-fixlen-array-count-bound-precedes-subtype/NOTES.md) | **the schema `count` bound is applied before the fixlen subtype decides the field is skippable** — an fp64-subtype array at a declared `fp32[5]` slot is skipped when in-count but `INVALID` when over-count (row 2), and a message truncated *between* the count word and the `fixlen_word` is `INVALID` instead of `INCOMPLETE` (row 4) | **corelib** — 7 on row 2 (csharp, dart, go, java, rust-std, rust-nostd, zig), 5 on row 4 (csharp, dart, go, java, rust-nostd); the array header hook fires before the `fixlen_word` or omits the subtype, so generated code cannot express the order at all | verdict | 🔴 **OPEN — filed 2026-07-29 against the seven corelibs that can fix it**: [corelib-go#58](https://github.com/sofa-buffers/corelib-go/issues/58), [corelib-java#53](https://github.com/sofa-buffers/corelib-java/issues/53), [corelib-cs#45](https://github.com/sofa-buffers/corelib-cs/issues/45), [corelib-dart#23](https://github.com/sofa-buffers/corelib-dart/issues/23), [corelib-rs-no-std#60](https://github.com/sofa-buffers/corelib-rs-no-std/issues/60) (rows 2+4) · [corelib-rs#40](https://github.com/sofa-buffers/corelib-rs/issues/40), [corelib-zig#27](https://github.com/sofa-buffers/corelib-zig/issues/27) (row 2 only — their hook already fires past the `fixlen_word`, so row 4 is correct there). [generator#232](https://github.com/sofa-buffers/generator/issues/232) — where this lived from 2026-07-25 as an open *spec question* — is **closed as misfiled**: no generator change can express the order. **Assigned this F-number 2026-07-29**; it had none. The spec question it was blocked on is **settled**: CORELIB_PLAN §4.8 (documentation#30, on `main` in `8087f1d`) fixes the order — count (format ceiling only, no allocation) → `fixlen_word` → §7.3 skip **without** applying the schema `count` → bound only for a survivor; truncation between the two words is `INCOMPLETE`. So it is now a pure implementation task, and a **corelib hook-signature** one. Three of the six vectors are unanimous (over-count + *matching* subtype → all `R`, with and without payload; the valid control round-trips on all 13), which isolates the defect to the ordering alone. Row 1 (java/csharp size the declared field from the skipped header's count) is **F-0039** / [generator#254](https://github.com/sofa-buffers/generator/issues/254), same hook, codegen — kept separate. **Re-measured 2026-07-29** on corelibs 0.9.0 + sofabgen 0.21.0: unchanged by the refactoring |
...
| G-0023 (= F-0039) | [generator#254](https://github.com/sofa-buffers/generator/issues/254) | 🔴 **open** — the java and csharp backends resize a declared native array from the count of a header §7.3 says to skip: `arrayBegin` lumps `UNSIGNED`/`SIGNED` into one skip-arm case and allocates the destination regardless of the wire kind, so a mistyped array decodes as a 1-element array of zeros. The same block applies the schema `count` bound before the kind check, which §7.3 inverts. Found 2026-07-29. Crucible finding **F-0039** |
| G-0024 (= F-0038) | [generator#257](https://github.com/sofa-buffers/generator/issues/257) | 🔴 **open** — the generated string visitor materializes and UTF-8-validates the payload *before* resolving the destination, so a **skipped** field (unknown id, or a §7.3 wire-type contradiction) is validated and the sticky invalid flag is set although no arm matches (CORELIB_PLAN §6.4 forbids it). rust-std, rust-no-std, java, csharp are pure codegen; go and dart are the codegen half of a two-half change with their corelibs. **zig already emits the correct shape** — destination switch first, `utf8_valid` inside the matched arm. Supersedes four misfiled corelib issues. Crucible finding **F-0038** |
