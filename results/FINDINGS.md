...
| [F-0054](../findings/F-0054-idmax-on-sequence-end-header/NOTES.md) | **the `ID_MAX` ceiling and a *sequence-end* header's id** — `76 87 80 80 80 40` (6 B): a skipped unknown sequence closed by an end marker whose header id is **2³¹**, one past `ID_MAX`. Four accept and re-encode to the empty message, nine reject. At `ID_MAX` and below all 13 accept, so **exactly one test point** is in dispute | **corelib-go, corelib-py, corelib-ts** (4 drivers, 3 repos) | verdict | 🔴 **open — found 2026-08-03; attribution moved twice the same day, see the finding's History.** Cluster of 4, minimized 56 B → 31 B → rebuilt as a 6-byte isolate; re-measured against every corelib's current `main`, verdicts unchanged. **Basis is Option B, pending [documentation#35](https://github.com/sofa-buffers/documentation/pull/35) — the spec's merged `main` still carries Option A, under which the other nine are at fault.** B keeps §4.9's encoder rule (`0x07`) and has the decoder **discard** the id but still bound it by `ID_MAX` like every other header: a bound on the id's *value*, not its spelling, so §4.1 is untouched and `0x87 0x00` / id 3 stay valid. That is what separates B from the abandoned Option C. B is also the only option that **removes** branching — the nine already carry one unconditional `if (id > ID_MAX)` before the wire-type dispatch. The three are **not** one shared gap: ts never computes the id for an end marker (`src/decode/fast.ts:92-101`), py computes it and checks it *after* the seq-end branch (`src/sofab/decoder.py:309-317`), and go carries a **written-out exception** (`cursor.go:272`, `t != TypeSequenceEnd &&`) — it had coded A before A existed. **Filed three times.** First against the four (go#67, py#58, ts#83 — closed on Option C, then marked superseded), then against the nine when A merged (corelib-c-cpp#128, corelib-cpp#68, corelib-cs#51, corelib-dart#30, corelib-java#60, corelib-rs#45, corelib-rs-no-std#66, corelib-zig#33 — **all eight and their draft PRs now closed `not planned`**). Current, on B: **[corelib-go#69](https://github.com/sofa-buffers/corelib-go/issues/69), [corelib-py#59](https://github.com/sofa-buffers/corelib-py/issues/59), [corelib-ts#85](https://github.com/sofa-buffers/corelib-ts/issues/85)** — each states that #35 is open and asks that the fix not land before it merges. |
