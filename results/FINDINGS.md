...
| [F-0038](../findings/F-0038-skipped-string-utf8-validated/NOTES.md) | **six corelibs UTF-8-validate a string field they are *skipping*** — a 3-byte isolate (`4a 0a 8a`: unknown id 9, fixlen subtype string, payload `0x8a`) is `A` on 7 and `R invalid_msg` on 6. CORELIB_PLAN §6.4 is normative and explicit: *"Skipped fields are never validated … UTF-8 validation runs only where a `string` is **materialized** … never on skip, in any mode"* — so the rejecters violate a MUST | corelib-go, corelib-rs, corelib-rs-no-std, corelib-java, corelib-cs, corelib-dart (6); **corelib-only** — the field id is unknown to the schema, so only the corelib's skip path sees it | verdict | 🔴 **OPEN — filed 2026-07-29** (6 issues): [corelib-go#57](https://github.com/sofa-buffers/corelib-go/issues/57), [corelib-rs#39](https://github.com/sofa-buffers/corelib-rs/issues/39), [corelib-rs-no-std#59](https://github.com/sofa-buffers/corelib-rs-no-std/issues/59), [corelib-java#52](https://github.com/sofa-buffers/corelib-java/issues/52), [corelib-cs#44](https://github.com/sofa-buffers/corelib-cs/issues/44), [corelib-dart#22](https://github.com/sofa-buffers/corelib-dart/issues/22). **Re-verified 2026-07-29** against corelibs **0.9.0 @ main** + sofabgen **0.21.0** — the first family carrying the merged sparse-array rewrite (documentation#29/#31). Still reproduces on both isolates. Three controls isolate the axis: same id with **valid** UTF-8 → all 13 `A` (not the unknown id); same byte with subtype **blob** → all 13 `A` (only `string` triggers it); the same byte in a **declared** field → all 13 `R` (the check itself is right, its *placement* is wrong). A §7.3 variant (`d6 0c 02 12 ff ff 07`, fixlen string at a `struct_array` element slot) splits the same six, so unknown-id and mistyped-skip share the root cause. Neighbour precedent **F-0012**: corelib-ts validating the *fixlen word* while skipping was the correct half — framing must be checked on skip, **content** must not. **Found 2026-07-27 by the 60-min pacemaker round** (105.6 M execs, corpus 890 → 1121); clusters 6 + 13 of the post-fuzz triage. |
